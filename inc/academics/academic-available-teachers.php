...
function educore_academics_available_teachers_view() {
    ...
    // Wrap the entire view in a Throwable catch block to prevent blank screens
    try {
        ...
        // --------------------------------------------------------------------------
        // 1. Auto-Create Proxy Log Table for Payroll & Audit safely
        // --------------------------------------------------------------------------
        ...
        if ( 'POST' === $req_method && isset( $_POST['educore_assign_proxy'] ) ) {
            if ( ! isset( $_POST['educore_proxy_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['educore_proxy_nonce'] ) ), 'assign_proxy_action' ) ) {
                $error_notice = esc_html__( 'Security validation failed. Please try again.', 'ifsedu-school-management' );
            } else {
                ...
                if ( 0 < $proxy_teacher_id && 0 < $unit_id ) {
                    ...
                    if ( false !== $inserted ) {
                        ...
                        $error_notice = esc_html__( 'Database write failed: ', 'ifsedu-school-management' ) . $wpdb->last_error;
                    }
                } else {
                    $error_notice = esc_html__( 'Please select a valid proxy teacher and target class.', 'ifsedu-school-management' );
                }
            }
        }
        ...
        // Load ONLY teachers (filtering by staff_type or designation containing 'Teacher' or 'Faculty')
        $teachers = $wpdb->get_results( "SELECT id, full_name, designation, phone FROM `{$table_staff}` WHERE status = 'Active' AND (staff_type LIKE '%Teacher%' OR staff_type LIKE '%Faculty%' OR designation LIKE '%Teacher%') ORDER BY full_name ASC" );
        if ( empty( $teachers ) ) {
        ...
        // Fetch Logged Proxy Records with Native Pagination
        $logs_per_page = 15;
        // phpcs:disable WordPress.Security.NonceVerification.Recommended
        ...
        $total_logs  = $wpdb->get_var( "SELECT COUNT(id) FROM `{$table_proxy}`" );
        ...
        $proxy_logs = $wpdb->get_results(
            $wpdb->prepare(
                "SELECT p.*, pt.full_name as proxy_teacher_name, ot.full_name as original_teacher_name, 
                 ...
                 ORDER BY p.proxy_date DESC, p.id DESC 
                 LIMIT %d OFFSET %d",
                $logs_per_page,
                $offset
            )
        ...
        <style id="ifs-educore-available-teachers-styles">
            ...
            .ifs-educore-notice-banner {
                background: #ecfdf5; border-left: 4px solid #00523c; color: #065f46;
                padding: 12px 16px; border-radius: 8px; font-weight: 700; margin-bottom: 20px;
                display: flex; align-items: center; gap: 8px;
            }
            .ifs-educore-error-banner {
            ...
            @media (max-width: 1080px) {
                .ifs-educore-proxy-grid { grid-template-columns: 1fr; }
            }
            .ifs-educore-proxy-card {
                background: #ffffff; border: 1px solid #e2e8f0; border-radius: 14px;
                padding: 22px; box-shadow: 0 4px 15px rgba(0, 0, 0, 0.02); box-sizing: border-box;
            }
            .ifs-educore-field-label {
                display: block; font-size: 12px; font-weight: 700; color: #475569;
                text-transform: capitalize; letter-spacing: 0.3px; margin-bottom: 6px;
            }
            .ifs-educore-field-input, .ifs-educore-field-select, .ifs-educore-field-textarea {
                width: 100%; border: 1.5px solid #cbd5e1; border-radius: 8px; font-size: 13.5px;
                color: #0f172a; background: #ffffff; box-sizing: border-box; outline: none;
                transition: border-color 0.2s, box-shadow 0.2s;
            }
            input.ifs-educore-field-input, select.ifs-educore-field-select {
                height: 40px; padding: 0 12px;
            }
            .ifs-educore-field-input:focus, .ifs-educore-field-select:focus, .ifs-educore-field-textarea:focus {
                border-color: #00523c; box-shadow: 0 0 0 3px rgba(0, 82, 60, 0.12);
            }
            .ifs-educore-status-pill-free {
                background: #ecfdf5; color: #047857; padding: 4px 10px; border-radius: 6px;
                font-weight: 700; font-size: 12px; display: inline-flex; align-items: center; gap: 4px; border: 1px solid #a7f3d0;
            }
            .ifs-educore-status-pill-busy {
                background: #fef2f2; color: #dc2626; padding: 4px 10px; border-radius: 6px;
                font-weight: 700; font-size: 12px; display: inline-flex; align-items: center; gap: 4px; border: 1px solid #fca5a5;
            }
            .ifs-educore-btn-proxy-action {
                background: #00523c; color: #ffffff; border: none; padding: 6px 12px;
            ...
            .ifs-educore-btn-proxy-action:hover { background: #065f46; }
            ...
            .ifs-educore-modal-backdrop.ifs-educore-is-visible { display: flex; }
            .ifs-educore-modal-box {
                background: #ffffff; border-radius: 14px; padding: 24px; width: 100%; max-width: 520px;
            ...
            .ifs-educore-modal-title { margin: 0; font-size: 16px; font-weight: 800; color: #0f172a; display: flex; align-items: center; gap: 6px; }
            .ifs-educore-modal-close { background: none; border: none; font-size: 22px; cursor: pointer; color: #64748b; }
            .ifs-educore-modal-teacher-badge { background: #f0fdf4; border: 1px solid #bbf7d0; padding: 10px 14px; border-radius: 8px; margin-bottom: 14px; }
            .ifs-educore-modal-field { margin-bottom: 12px; }
            .ifs-educore-modal-actions { display: flex; justify-content: flex-end; gap: 8px; }
            .ifs-educore-btn-cancel { background: #f1f5f9; border: none; padding: 8px 14px; border-radius: 6px; font-weight: 700; cursor: pointer; color: #475569; }
            .ifs-educore-btn-confirm { background: #00523c; color: #fff; border: none; padding: 8px 18px; border-radius: 6px; font-weight: 700; cursor: pointer; }
            .ifs-educore-filter-form-wrap { display: flex; gap: 14px; align-items: flex-end; flex-wrap: wrap; }
            
            /* Table & DataTables overrides */
            .ifs-educore-table { width: 100%; border-collapse: collapse; font-size: 13px; text-align: left; }
            .ifs-educore-table th { padding: 10px 12px; background: #f8fafc; border-bottom: 2px solid #e2e8f0; font-size: 11.5px; font-weight: 800; color: #475569; text-transform: capitalize; }
            .ifs-educore-table td { padding: 10px 12px; border-bottom: 1px solid #f1f5f9; vertical-align: middle; }
            
            .dataTables_wrapper .dataTables_filter { margin-bottom: 10px; }
            .dataTables_wrapper .dataTables_filter input { border: 1px solid #cbd5e1; border-radius: 6px; padding: 4px 8px; outline: none; }
            .dataTables_wrapper .dataTables_filter input:focus { border-color: #00523c; }
            .dataTables_wrapper .dataTables_length select { border: 1px solid #cbd5e1; border-radius: 6px; padding: 2px 6px; }
            .dataTables_wrapper .dataTables_paginate .paginate_button { padding: 4px 10px !important; margin-left: 4px !important; border-radius: 6px !important; border: 1px solid #cbd5e1 !important; background: #fff !important; color: #475569 !important; font-size: 12px !important; }
            .dataTables_wrapper .dataTables_paginate .paginate_button.current { background: #00523c !important; color: #fff !important; border-color: #00523c !important; }
            .dataTables_wrapper .dataTables_paginate .paginate_button:hover { background: #f8fafc !important; color: #0f172a !important; }

            .ifs-educore-ledger-list { display: flex; flex-direction: column; gap: 10px; }
            .ifs-educore-ledger-item { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 12px; font-size: 12px; }
            
            /* Native WP Pagination for Proxy Logs */
            .ifs-educore-pagination { display: flex; gap: 6px; justify-content: center; margin-top: 20px; padding-top: 15px; border-top: 1px solid #e2e8f0; }
            .ifs-educore-pagination a, .ifs-educore-pagination span { display: inline-flex; align-items: center; justify-content: center; min-width: 32px; height: 32px; padding: 0 8px; border-radius: 6px; border: 1px solid #cbd5e1; background: #ffffff; color: #475569; font-size: 13px; font-weight: 600; text-decoration: none; transition: all 0.2s; }
            .ifs-educore-pagination a:hover { background: #f8fafc; border-color: #94a3b8; }
            .ifs-educore-pagination .current { background: #00523c; color: #ffffff; border-color: #00523c; }
        </style>
        ...
        <div class="ifs-educore-proxy-root">
            ...
            <?php if ( ! empty( $error_notice ) ) : ?>
                <div class="ifs-educore-error-banner">
                    <span class="dashicons dashicons-warning"></span> <?php echo esc_html( $error_notice ); ?>
                </div>
            <?php endif; ?>
            ...
            <div class="ifs-educore-proxy-grid">
                ...
                <div class="ifs-educore-proxy-card">
                    ...
                    <div style="overflow-x:auto;">
                        <table class="ifs-educore-table" id="ifs-proxy-teachers-table">
                            <thead>
                                <tr>
                            ...
                            <tbody>
                                ...
                                <?php endforeach; else : ?>
                                    <tr><td colspan="4" style="text-align:center; padding:30px; color:#94a3b8;"><?php esc_html_e( 'No active teachers found.', 'ifsedu-school-management' ); ?></td></tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
                    </div>
                ...
                <div class="ifs-educore-proxy-card">
                    ...
                    <div class="ifs-educore-ledger-list">
                    ...
                    <?php 
                    // Render PHP Pagination block for the Logs Ledger
                    if ( $total_pages > 1 ) {
                        ...
                        if ( $page_links ) {
                            echo '<div class="ifs-educore-pagination">' . $page_links . '</div>';
                        }
                    }
                    ?>
                </div>
        ...
        jQuery(document).ready(function($) {
            ...
            // Apply DataTables Pagination to the Teacher List
            if ($.fn.DataTable) {
                $('#ifs-proxy-teachers-table').DataTable({
                    pageLength: 10,
                    ...
                    columnDefs: [
                        { orderable: false, targets: 3 } // Prevent sorting on the action button column
                    ]
                });
            }
            ...
            // Bulletproof global document delegation for DataTables redraw persistence
            $(document).on('click', '.btn-open-proxy-modal', function(e) {
                e.preventDefault();
                var teacherId = $(this).attr('data-teacher-id');
                var teacherName = $(this).attr('data-teacher-name');
            ...
            // Close modal when clicking outside the box backdrop
            if (modal) {
        ...
        // Fallback layout if a fatal error occurs inside the function logic
        echo '<div style="background: #fef2f2; border: 1px solid #fca5a5; padding: 20px; border-radius: 8px; color: #991b1b; margin-top: 20px; max-width: 800px;">';
        echo '<h3 style="margin-top: 0; color: #991b1b; display: flex; align-items: center; gap: 8px;"><span class="dashicons dashicons-warning" style="font-size: 24px; height: 24px; width: 24px;"></span> System Diagnostic Error Captured</h3>';
        echo '<p style="font-size: 14px; margin-bottom: 8px;"><strong>Fatal Error:</strong> ' . esc_html( $e->getMessage() ) . '</p>';
        echo '<p style="font-size: 13px; color: #b91c1c; margin-bottom: 4px;"><strong>Line:</strong> ' . esc_html( $e->getLine() ) . '</p>';
        echo '<p style="font-size: 13px; color: #b91c1c; font-family: monospace;"><strong>File:</strong> ' . esc_html( $e->getFile() ) . '</p>';
        echo '</div>';
    }
}