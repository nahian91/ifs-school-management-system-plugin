...
function educore_daily_attendance_view( $classes, $sections, $filter_class, $filter_section, $filter_date ) {
    ...
    if ( isset( $_POST['educore_save_attendance'], $_POST['educore_attendance_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['educore_attendance_nonce'] ) ), 'save_attendance_action' ) ) {
        ...
        $notice_banner_html = '<div class="ifs-educore-alert-success"><span class="dashicons dashicons-yes-alt"></span> ' . sprintf(
    ...
    <div class="ifs-educore-attendance-daily-root">
        ...
        <div class="ifs-educore-bento-card no-print">
            <form method="GET" action="<?php echo esc_url( admin_url( 'admin.php' ) ); ?>" class="ifs-educore-filter-grid">
                ...
                <div class="ifs-educore-filter-node">
                    <label class="ifs-educore-filter-label"><?php esc_html_e( 'Select Target Date', 'ifsedu-school-management' ); ?> *</label>
                ...
                <div class="ifs-educore-filter-node">
                    <label class="ifs-educore-filter-label">
                        <?php esc_html_e( 'Academic Class', 'ifsedu-school-management' ); ?> *
                        <?php if ( ! $is_admin ) : ?>
                            <span class="ifs-educore-filter-sublabel">(<?php esc_html_e( 'Assigned Only', 'ifsedu-school-management' ); ?>)</span>
                        <?php endif; ?>
                    </label>
                    <select name="class_name" id="educore_attendance_class_select" class="ifs-educore-select" required>
                ...
                <div class="ifs-educore-filter-node">
                    <label class="ifs-educore-filter-label">
                        ...
                        <?php if ( ! $is_admin ) : ?>
                            <span class="ifs-educore-filter-sublabel">(<?php esc_html_e( 'Assigned', 'ifsedu-school-management' ); ?>)</span>
                        <?php endif; ?>
                    </label>
                    <select name="section_name" id="educore_attendance_section_select" class="ifs-educore-select">
                        <option value=""><?php esc_html_e( '-- All Sections --', 'ifsedu-school-management' ); ?></option>
                    </select>
                </div>

                <div class="ifs-educore-filter-node-wide">
                    <label class="ifs-educore-filter-label"><?php esc_html_e( 'Student (Optional)', 'ifsedu-school-management' ); ?></label>
                    <select name="filter_student" id="educore_attendance_student_select" class="ifs-educore-select">
                        <option value=""><?php esc_html_e( '-- All Students --', 'ifsedu-school-management' ); ?></option>
                    </select>
                </div>
                
                <div>
                    <button type="submit" class="ifs-educore-btn-load"><?php esc_html_e( 'Load Students', 'ifsedu-school-management' ); ?></button>
                </div>
            </form>
        </div>
        ...
        if ( ! empty( $filter_class ) ) {
            ...
            if ( $students ) {
                ...
                <div class="ifs-educore-bento-card" style="padding:0; overflow:hidden;">
                    ...
                    <div class="ifs-educore-roster-meta-bar">
                        <div class="ifs-educore-roster-title">
                            <h4 class="ifs-educore-roster-title-main"><?php esc_html_e( 'Mark Attendance:', 'ifsedu-school-management' ); ?> <span class="ifs-educore-roster-class-highlight"><?php echo esc_html( $filter_class . ( $filter_section ? ' (' . $filter_section . ')' : '' ) ); ?></span></h4>
                            <small class="ifs-educore-roster-date-sub"><?php esc_html_e( 'Target Date:', 'ifsedu-school-management' ); ?> <?php echo esc_html( $att_date_formatted ); ?></small>
                        </div>
                        
                        <div class="ifs-educore-counter-cluster">
                            <span class="ifs-educore-counter-badge-total"><?php esc_html_e( 'Total:', 'ifsedu-school-management' ); ?> <span id="cnt-total"><?php echo absint( count( $students ) ); ?></span></span>
                    ...
                    <form method="POST" action="" id="educoreAttendanceSubmitEngine">
                        ...
                        <div style="overflow-x:auto;">
                            <table class="ifs-educore-attendance-table">
                                <thead>
                                    <tr>
                                        <th style="width: 12%;"><?php esc_html_e( 'Roll No', 'ifsedu-school-management' ); ?></th>
                                        <th style="width: 18%;"><?php esc_html_e( 'Student ID', 'ifsedu-school-management' ); ?></th>
                                        <th style="width: 35%;"><?php esc_html_e( 'Student Name', 'ifsedu-school-management' ); ?></th>
                                ...
                                <tbody>
                                    ...
                                    <tr class="student-attendance-row">
                                        <td><strong class="ifs-educore-roll-strong"># <?php echo esc_html( $student->roll_no ); ?></strong></td>
                                        <td><code class="ifs-educore-student-id-code"><?php echo esc_html( strtoupper( (string) $student->student_id ) ); ?></code></td>
                                        <td><span class="ifs-educore-student-name-text"><?php echo esc_html( $student->full_name ); ?></span></td>
                        ...
                        <div class="ifs-educore-submit-bar">
                            <button type="submit" name="educore_save_attendance" class="ifs-educore-btn-save-att">
                                <span class="dashicons dashicons-saved" style="margin-top:2px;"></span> <?php esc_html_e( 'Save Attendance Data', 'ifsedu-school-management' ); ?>
                            </button>
                        </div>
                    </form>
                </div>
                ...
                echo '<div class="ifs-educore-alert-warning"><span class="dashicons dashicons-warning" style="font-size:24px; width:24px; height:24px; margin-bottom:10px; display:block; margin:0 auto;"></span><p style="margin:0;">' . esc_html__( 'No active students found matching current filters.', 'ifsedu-school-management' ) . '</p></div>';
            }
        } else {
